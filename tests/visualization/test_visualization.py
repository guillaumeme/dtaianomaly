...
class TestPlottingFunctions:
    ...
    def test_multivariate(self, plot_function, additional_args, obligated_y_pred, multivariate_time_series):
        ...
        plot_function(multivariate_time_series, y, **additional_args)
    ...
    def test_give_time_steps(self, plot_function, additional_args, obligated_y_pred, univariate_time_series):
        ...
        if obligated_y_pred:
            additional_args['y_pred'] = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        plot_function(univariate_time_series, y, time_steps=time_steps, **additional_args)
...
class TestPlotWithZoom:
    ...
    def test(self, plot_function, additional_args, obligated_y_pred, univariate_time_series):
        y = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        if obligated_y_pred:
            additional_args['y_pred'] = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        visualization.plot_with_zoom(univariate_time_series, y, start_zoom=100, end_zoom=200, method_to_plot=plot_function, **additional_args)

    def test_given_time_steps(self, plot_function, additional_args, obligated_y_pred, univariate_time_series):
        y = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        time_steps = visualization.format_time_steps(None, univariate_time_series.shape[0]) * 2 + 10
        if obligated_y_pred:
            additional_args['y_pred'] = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
