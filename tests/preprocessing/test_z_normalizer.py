...
from dtaianomaly.preprocessing import ZNormalizer
...
class TestZNormalizer:
    ...
    def test_distribution(self, univariate_time_series):
        x_, _ = ZNormalizer().fit_transform(univariate_time_series)
        assert x_.mean() == pytest.approx(0.0)
        assert x_.std() == pytest.approx(1.0)

    def test_single_value(self):
        x = np.ones(1000) * 123.4
        assert np.array_equal(ZNormalizer().fit_transform(x)[0], x)
